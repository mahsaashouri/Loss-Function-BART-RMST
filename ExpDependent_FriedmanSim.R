...
Bias[,1] <- mean(na.omit(bias_bart_dep))
Bias[,2] <- mean(na.omit(bias_bart_dep_default))
Bias[,3] <- mean(na.omit(bias_aft_bart))
Bias[,4] <- mean(na.omit(bias_aft_bart_default))
Bias[,5] <- mean(na.omit(bias_bcart))
Bias[,6] <- mean(na.omit(bias_bcart_default))
Bias[,7] <- mean(na.omit(bias_bart))
Bias[,8] <- mean(na.omit(bias_bart_default))
Bias[,9] <- mean(na.omit(bias_coxph))
Bias[,10] <- mean(na.omit(bias_rcoxph))
Bias[,11] <- mean(na.omit(bias_ipcw))
Bias[,12] <- mean(bias_aft, na.rm = T)
Bias[,13] <- mean(na.omit(bias_aft_null))
